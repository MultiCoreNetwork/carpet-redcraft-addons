# Creating all the scoreboards that are needed by the datapack
    ...
    # Adding the pack name and version to the redpack storage
        ...
        data modify storage redpack:config versions append value {"id":128358,"name":"RedPack - Craftable Deepslate Ores","version":"0.1.0"}
    ...
    # Getting the version of the player and sending an error message if it is outdated
        ...
        execute if score $gb.game_version gb.configs matches ..2695 run tellraw @a[tag=!global.ignore.gui] {"text":"[Craftable Deepslate Ores (RedPack)]> This datapack is not compatible with all the versions before 21w08a!\n","color":"red"}
