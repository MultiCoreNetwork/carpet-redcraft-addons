# Creating all the scoreboards that are needed by the datapack
    ...
    # Adding the pack name and version to the redpack storage
        data remove storage redpack:config versions[{id:128308}]
        data modify storage redpack:config versions append value {"id":128308,"name":"RedPack - Quartz Crafting Compatibility","version":"0.1.0"}
    ...
    # Getting the version of the player and sending an error message if it is outdated
        ...
        execute if score $gb.game_version gb.configs matches ..2576 run tellraw @a[tag=!global.ignore.gui] {"text":"[Quartz Crafting Compatibility (RedPack)]> This datapack is not compatible with all the versions before 1.16.2!\n","color":"red"}
